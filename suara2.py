...
dfr_obj = ADC(Pin(34))
dfr_obj.atten(ADC.ATTN_11DB)       #Full range: 3.3v 
...
def read_sound():
    ...
    sound_value = dfr_obj.read()
    if sound_value < 250 :
        print ('Keadaan Hening')
    elif 251 < sound_value > 700 :
        print('Tingkat Kebisingan Sedang')
    else :
        print('Tingkat Kebisingan Tinggi')
def connect():
    # Connect to WiFi
...
def subscribe_command2():
    ...
    #mqtt_topic = '/devices/{esp_coba}/commands/#'
    mqtt_topic = '/devices/{}/{}'.format(DEVICE_ID, 'commands/#')
    commands = 'Minta Keadaan Sekarang'
    data = commands.encode("utf-8")
    while True:
        sound_value = dfr_obj.read()
        if sound_value < 250:
...
#  Read from DHT22
#print("Reading from Sound Sensor")
#result = read_sound()
# Publish a message
